{{-- Lokasi File: resources/views/pages/services.blade.php --}}

<x-main-layout>
    @section('title', 'Our Services - Rumah Selam Lembeh')
    @section('description', 'Explore the variety of diving packages, land tours, and transportation services offered by Rumah Selam Lembeh Dive Center.')

    {{-- ========================================================== --}}
    {{-- Hero Section with Slider --}}
    {{-- !! PERUBAHAN DI SINI: Atur posisi absolute dan tinggi !! --}}
    {{-- ========================================================== --}}
    <section
        ...
        x-init="init()"
        {{-- Jadikan absolut, tempel ke atas, beri tinggi (misal 75vh/80vh) --}}
    ...
    >
        <template x-for="(image, index) in images" :key="index">
            {{-- Path gambar menggunakan logika dari Anda ('/' + image) --}}
            <div
                class="absolute inset-0 bg-cover bg-center"
                :style="'background-image: url(\'/' + image + '\');'"
                x-show="current === index"
                {{-- Tambahkan transisi fade (opsional tapi bagus) --}}
        ...
        <div class="relative z-10 text-center px-4 animate-fade-in-up">
            <h1 class="text-4xl md:text-5xl font-extrabold">{{ __('services.header.title') }}</h1>
            <p class="text-lg md:text-xl mt-2 text-gray-200">{{ __('services.header.description') }}</p>
        </div>
    </section>
                                        ...
                                        <div x-data="{ open: false }" class="bg-white rounded-xl shadow-lg overflow-hidden transition-shadow duration-300 hover:shadow-2xl">
                                            <div class="md:flex">
                                                ...
                                                <div class="p-8 flex flex-col flex-grow">
                                                    {{-- Kembalikan ke title (sesuai kode Anda) --}}
                                                    <h3 class="text-2xl font-bold text-blue-600 mb-3">{{ $service->title }}</h3>

                                                    {{-- Kembalikan ke description (sesuai kode Anda) --}}
                                                    @if($service->description)
                                                        {{-- Tampilkan sebagai HTML biasa --}}
                                                    ...
                                                    <div class="mt-auto pt-6 flex flex-wrap items-center gap-4">
                                                        ...
                                                        {{-- Kembalikan ke content (sesuai kode Anda) --}}
                                                        @if($service->content)
