                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('admin.reviews.update', $review->id) }}">
                        ...
                        @foreach(config('app.available_locales') as $locale => $localeName)
                            <div class="mb-4">
                                ...
                                <x-text-input
                                    ...
                                    :value="old('reviewer_name.' . $locale, $review->getTranslation('reviewer_name', $locale))"
                                    :required="$locale === 'en'"
                                    autofocus="{{ $locale === 'en' }}" />
                         ...
                         @foreach(config('app.available_locales') as $locale => $localeName)
                            <div class="mb-4">
                                ...
                                <textarea
                                    ...
                                    rows="5"
                                    {{ $locale === 'en' ? 'required' : '' }}
                                >{{ old('review_text.' . $locale, $review->getTranslation('review_text', $locale)) }}</textarea>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="rating" class="block mt-1 w-full" type="number" name="rating" :value="old('rating', $review->rating)" required min="1" max="5" />
                            @error('rating')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
